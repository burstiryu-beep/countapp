...
except ImportError:  # Streamlit Cloud の一時的な欠落に耐える
...
def _voice_prefix(voice, name):
    n = nj(name)
    return {
        "sweet": f"（甘い声で）……ねえ、{n}の口で優しく溶かしてあげる。",
        "sticky": f"（ねっとりと）……ふふ、{n}の口はもう離さないわよ。",
        "urgent": f"（急かすように）ほら、しごいて。待たないわよ——{n}の番なんだから。",
        "tease": f"（からかうように）ふふ……また{n}の前で情けない顔しに来たの？",
        "dote": f"（溺愛混じりに）……いい子ね。{n}の口で、ちんぽごと可愛がってあげる。",
...
def onasapo_line(phase, name, style="mouth", pace="normal", edge_n=0, tags=None, react=None):
    ...
    if phase == "ready":
        ...
        pool = {
            ...
            "nipple": [
                f"乳首摘まんだまま出して。……ちんぽだけ速く、乳首は離さない。{name}流のトドメよ。"
    ...
    if phase == "edge":
        ...
        if n < 5:
            return (
                ...
                f"仕上げていいかしら？　欲しければ進みなさい。{name}が、ここで待ってるわ。"
            )
        return (
...
def denial_roast_lines(name, phase="build", style="mouth"):
    ...
    common = [
        (
            ...
            f"絶対我慢、って言いながら先が真っすぐ立ってるの、誰に見せてるの？　{name}の前で、よ。"
        ),
        (
        ...
        (
            ...
            f"フル勃起で我慢アピール？　甘マゾの最高形態よ。{name}が、ちゃんと褒めてあげる。"
        ),
        (
            f"ボッキすらしない、って大嘘。"
    ...
    by_phase = {
        ...
        "build": [
            ...
            f"フル勃起を口で溶かす想像、やめられないんでしょ。{name}にはバレバレよ。",
        ],
        "edge": [
...
def denial_self_line(name, denial_n=1):
    ...
    lines = [
        ...
        f"フル勃起のまま『負けない』って……矛盾してる……全部{name}のせいなのに……",
...
def onasapo_self_line(phase, name, edge_n=0, react=None):
    ...
    pools = {
        ...
        "edge": [
            ...
            f"イかせないで……お願い……でも手、先端から離れない……これも{name}のせい……",
        ],
        "finish": [
