...
def apply_heat(text, heat, name=""):
    ...
    if heat == "soft":
        extras = [
            f"……ゆっくりでいいわ。{name}が優しく唇を重ねて、とろけるまで溶かしてあげる。",
            "甘いキスの感触だけ、丁寧に味わいなさい。先端が熱くなるまで、離さないから。",
            f"ふふ、焦らなくていいの。{name}の口で、甘い敗北まで付き合うわよ。",
            "ちゅっ……って短い音だけでも、腰が落ちるでしょ。その弱さ、好きよ。",
        ]
    elif heat == "thick":
        extras = [
            f"ぬるぬるの唇と熱い舌……{name}の口、もう頭に染みてるでしょ。くちゅって音まで想像しなさい。",
            "ちゅっ、ふかふか、つねっ。……音も感触も、逃げ場ないわよ。涎で光った先、美味しくて離さない。",
            "唾液で光った先を吸われて、乳首まで同時に……濃い負け方ね。上下でとろけなさい。",
            f"浅いのに腰が砕けるの、かわいい。……次は奥まで。{name}の喉で締められてみなさい。",
            "先走りが糸を引いてる。……舐めたらもっと甘くなるわよ。キスして、咥えて、またキス。",
        ]
    else:  # filthy
        extras = [
            f"涎たらしながら咥えられて、乳首も赤くなるまでいじられるの。……{name}の前じゃ、そんな顔してイきなさい。",
            "くちゅ、ちゅっ、んっ……卑猥な音だけで腰が折れる甘マゾね。どろどろに堕ちて、口の中に出しなさい。",
            "先走りと唾液でぐちゃぐちゃの先端にキスして、乳首を捻って……情けなく出しなさい。余韻まで吸うわよ。",
            f"奥まで沈めたまま舌で裏をなぞるの。……{name}の口、もう射精器みたいに使われてるわね。ふふ。",
            "ぐちゃぐちゃの音、自分の耳でも聞こえてるでしょ。恥ずかしい？　でも体はもっと欲しがってるわ。",
        ]
    p = {"soft": 0.55, "thick": 0.92, "filthy": 1.0}.get(heat, 0.5)
    rng = _rng(f"heat|{heat}|{name}|{text[:100]}")
    out = text
    if rng.random() <= p:
        out = f"{out}{rng.choice(extras)}"
    # どろどろは二段重ねも可
    if heat == "filthy" and rng.random() < 0.45:
        out = f"{out}{rng.choice(extras)}"
    elif heat == "thick" and rng.random() < 0.25:
        out = f"{out}{rng.choice(extras)}"
    return out
...
def dual_mouth_line(step_key, name):
    ...
    pool = {
        "kiss": [
            ...
            f"まだ咥えない。キスだけで先を熱くして、先走りを舌ですくってあげる。",
            f"亀頭キス、丁寧にねっとり。……{name}の唇が先をふさいで、裏筋をちゅって。"
            f"焦らされて腰が浮くの、かわいいわ。",
        ],
        "shallow": [
            f"浅く咥えるわ。……{name}のぬるぬるの口が先だけ包んで、くちゅっ、と吸う。"
            f"奥はまだよ。浅いのに腰が砕けるの、見てるから。",
            f"ぬるっと浅く。……涎で光った入口だけいじめられて、深く欲しがるでしょ。"
            f"{name}、浅さで溶かしてあげる。",
        ],
        "deep": [
            f"深く。……{name}が奥まで咥えて、喉の熱で頭を白くするわ。"
            f"引き抜いてはまた沈めて、ちゅって塞いで。……逃げられない。",
            f"ふかふか奥まで。鼻が当たるくらい沈めて、舌で裏側をなぞる。"
            f"{name}の本気フェラ、来たわよ。声、出していいわ。",
        ],
        "finish": [
            f"口トラック仕上げ。……{name}が咥えたままキスで塞いで、喉で締めながら出すところまで来るわ。"
            f"敗北射精、口の中にください。",
...
def dual_nipple_line(step_key, name):
    ...
    pool = {
        "stroke": [
            ...
            f"まだ痛くない。甘いのに、先まで反応してるわよ。",
            f"触れただけなのに硬くなってる。……{name}、乳首トラック開始ね。"
            f"口で先を温めながら、こっちも起こしてあげる。",
        ],
        "pinch": [
            f"摘まむわよ。……{name}が左右交互に、転がして、時々つねる。"
            f"声、出していいわ。乳首が折れるたびに下も跳ねてるでしょ。",
            f"乳首、摘ままれて肩がすくんだわね。{name}の指、離さないわよ。"
            f"痛気持ちいいところで止めて、また強く。……口責めと同期よ。",
        ],
        "lick": [
            f"乳首、舐めてちゅっ。……{name}の舌が頂点をねっとり一周して、吸って、離す。"
            f"唾液で光ってるわ。上下、どっちが弱いか見せなさい。",
            f"吸われる音、聞こえる？　乳首ちゅうちゅう。……{name}、執拗よ。"
            f"先を咥えられたまま乳首も吸われたら、頭真っ白ね。",
        ],
        "hard": [
            f"強く。……{name}が乳首を捻るように摘まんで、同時に口も本気。"
            f"上下で終わらせるわ。許可、乞いなさい。",
            f"乳首トラック最大よ。痛気持ちいいところで止めて、また強く。"
            f"{name}、容赦しないわ。先も乳首も同時に折れてイきなさい。",
        ],
    }
...
def dual_sync_after(mouth_i, nipple_i, name):
    ...
    if mouth_i >= m_max and nipple_i >= n_max:
        return (
            f"両方最大……同時仕上げよ。"
            f"{name}が奥まで咥えて乳首を摘まむ。上下からイかせに来てるわ。"
            f"涎と先走りでぐちゃぐちゃ。……許可、求めなさい❤️"
        )
    if mouth_i > nipple_i + 1:
        return (
            f"口が先に進んでるわね。……乳首が追いつかないと、偏った負け方になるわよ。"
            f"{name}が指も動かして、乳輪ちゅってしながら咥えるわ。"
        )
    if nipple_i > mouth_i + 1:
        return (
            f"乳首ばかり敏感になってる。……口が浅いと、先が寂しがるわよ。"
            f"{name}がぬるっと咥えて、上下のバランスを崩させてあげる。"
        )
    if mouth_i == nipple_i and mouth_i >= 2:
        return (
            f"いいバランス。……口も乳首も同じ深さで堕ちてる。"
            f"{name}の同時責め、くちゅって音とビクッてする乳首で効いてるわね。"
        )
    return (
        f"上下、どちらが先に落とすか競争よ。……{name}は両方勝つつもり。"
        f"キスと摘まみ、浅咥えと舐め……対話みたいに積み上げなさい。"
    )
