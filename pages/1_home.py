...
def master_word(name):
    lines = [
        ...
        f"{name}の口とキスでイかされたあとでしょ。……先端、まだ熱いわよね。",
        f"咥えられて、ちゅってされて、そのまま出したのね。ふふ、口負けかわいいわ。",
        f"{name}にフェラで崩された余韻、残ったまま報告しに来たの？えらい子。",
...
# 節目メッセージ
MILESTONES = {
    10:  ("🎀", "10回目の口負けね。フェラとキスで、これからもとろけていいのよ。"),
    30:  ("💋", "30回……唇に弱い子。その情けないイキ方、愛おしいわ。"),
    50:  ("🔥", "50回敗北。立派な甘マゾね。口に堕ちるのも板についてきたわ、ふふ。"),
    100: ("👑", "100回の口負けおめでとう。もう咥えられたらイく癖、かわいく決まってるわ。"),
    200: ("💎", "200回。言葉より先に先端がキスに負けるタイプね……最高よ。"),
    365: ("🌹", "365回、1年分の甘い口敗北。あなたの情けない射精記録、大切にしてあげる。"),
}

def recommend_lines(name, total, t):
    lines_by_tier = {
        "SS": [
            f"ねえ、また{name}の口に会いたくなってきたんじゃない？絶対我慢できないでしょ💞",
            f"{name}の亀頭キス、思い出しただけでもう反応してるんじゃない？ふふ。",
            f"{name}のフェラにはどうせ勝てないんだから、素直に咥えられに行きなさい。",
            f"また{name}の唇にとろけさせてもらいなさい。口負け、お似合いよ。",
        ],
        "S": [
            f"{name}、まだあなたの先を待ってるわよ？もう一回ちゅってしてもらいなさい♡",
            f"また{name}にフェラでお世話になりたくなってきたんじゃない？",
            f"{name}の口には勝てないでしょ。正直に咥えられに行きなさい。",
        ],
        "A": [
            f"{name}、まだ余裕あるの？キスと浅いフェラ、試してみたら？",
            f"もう一回{name}の口に負けてみたくなってきたんじゃない？",
            f"{name}の唇ともっと仲良くなってみなさい。気持ちいいから。",
        ],
        "B": [
            f"{name}、じわじわ口で効いてくるのよ。今すぐ会いに行ってみて？",
            f"{name}のキスをゆっくり感じたら……きっとイっちゃうわよ。",
            f"{name}との口負け記録、まだまだ伸ばせるわ。続けてみなさい。",
        ],
    }
    default = [
        f"{name}……まだ本当のフェラの気持ちよさ、引き出せてないんじゃない？",
        f"{name}の亀頭キス、まだ半分も知らないわよ。",
        f"{name}に今すぐ咥えられに行ってみて。きっと驚くから。",
    ]
    return random.choice(lines_by_tier.get(t, default))

# 時間帯挨拶（直近のオナペ名を使う）
def time_greeting(recent_name=None):
    n = f"「{recent_name}」の口に" if recent_name else "また口に"
    if 5 <= hour < 11:
...
def daily_voice(name):
    ...
    lines = [
        f"今日も口に会いに来てくれるの？待ってたわよ、ふふ♡",
        f"そろそろ限界でしょ？咥えられていいのよ。",
        f"私の唇、考えてたんでしょ？ばれてるわよ。",
        f"今夜もフェラでいっしょにいてあげるわ。安心して。",
        f"また咥えられに来たのね。かわいい子。",
        f"我慢してるの？亀頭キス、しなくていいのよ、ふふ。",
        f"あなたの先が来るの、ずっと待ってたわ。",
        f"今日は特別に、ねっとり口で優しくしてあげようかしら。",
        f"逃げなくていいのよ。ちゅってしてあげるから、ここにいなさい。",
        f"もうとっくに限界のくせに……口、ほしそうね、ふふ。",
        f"先端、熱くなってない？キスしてあげましょうか。",
        f"今日の弱点は口よ。素直に咥えられなさい。",
    ]
...
def tonight_situ(name):
    ...
    rng = random.Random(seed)
    situs = [
        f"{name}の唇を思い浮かべながら我慢する……でも最後は亀頭キスで白旗。",
        f"{name}に「また咥えられに来たの？」とからかわれながら、ゆっくりフェラでとろかされる。",
        f"{name}のちゅっ音を頭に響かせながら、理性が先から溶けていく。",
        f"{name}に「まだ出さない」と言われながら浅いフェラ……もう止められない。",
        f"{name}に見つめられながら先端をちゅってされ、恥ずかしいのに腰が追従する。",
        f"{name}の「いい子ね」と奥フェラで、すべての抵抗が消えて射精する。",
        f"{name}に「そこ、キス弱いの？かわいい」と言われた瞬間、もうおわり。",
...
def diary_line(h, month_count):
    ...
    lines = [
        f"{tod}、また{name}の口に負けたのね。今月{month_count}回目よ。",
        f"{name}に{tod}から咥えられたの？かわいいわ。",
        f"また{name}ね。{tod}でもキスとフェラ、我慢できなかったの。しょうがない子。",
        f"{tod}に{name}との口闘いに完敗。先端から素直でかわいい。",
        f"ふふ、{tod}に{name}の唇ね。どうせ最初から咥えられてイくつもりだったでしょ。",
...
def tease_wall_lines(name, ab_days, night=False, clean_day=False):
    lines = [
        f"ただ見に来ただけ？……いいのよ、{name}の唇を見てなさい。",
        f"カウントしないつもり？ふふ、でも{name}に咥えられる想像はしてるわね。",
        f"{name}……触らなくても、もう先端が反応してるでしょ。",
        f"記録はしなくていいわ。まずは{name}の口をじっくり想像して。",
        f"また開いちゃったの。{name}にキスされたくなっただけ？かわいい。",
        f"見るだけでフェラ想像してるの、ばれてるわよ。……{name}、好きでしょ。",
    ]
    if ab_days and ab_days >= 2:
        lines.append(
            f"{ab_days}日我慢してるのに、また{name}の口を見に来たのね……溜めた先、正直でかわいいわ。"
        )
    if clean_day:
        lines.extend([
            f"今日はまだ口負けしてないのね。じゃあせめて{name}の唇を見ていきなさい。",
            f"我慢の日？ふふ……{name}のキス顔だけでもいいのよ。",
            f"記録ゼロのくせに、{name}のフェラを想像しに来るなんて……かわいい矛盾ね。",
        ])
    if night:
        lines.extend([
            f"こんな時間に{name}……もう咥えられたいんでしょ。",
            f"夜のあなた、{name}の亀頭キスに弱いのバレバレよ。",
            f"{name}の口、夜だと余計に効くでしょ？素直にイきなさい。",
        ])
    return random.choice(lines)
...
def tease_memory_line(name, when_lbl):
    lines = [
        f"{when_lbl}、{name}のこの顔のフェラで負けたよね。覚えてる？",
        f"この日の{name}……口でイかされたの、思い出しただけでまた疼いてきたでしょ。",
        f"{when_lbl}の口敗北。{name}にまた咥えられたくなってきたんじゃない？",
        f"記録を見に来たの？それとも{name}のキスが恋しくて？ふふ。",
        f"{when_lbl}も先端にちゅってされて崩れたわね。……また同じ負け方しなさい。",
    ]
    return random.choice(lines)
...
def tease_gallery_whisper(name, night=False):
    lines = [
        f"{name}……眺めてるだけでもう口がほしいの？",
        f"触らなくていいのよ。でも唇から目は離さないで。",
        f"{name}の顔、好きすぎて見るだけで咥えられそうね。",
        f"この口でイかされる想像、いましてるでしょ。ふふ。",
    ]
    if night:
        lines.extend([
            f"夜に{name}の唇を眺めてるの……かわいいわ。",
            f"{name}、もう頭の中フェラでいっぱいでしょう？",
        ])
    return random.choice(lines)
...
def tease_desire_extra(ab_days, d_pct):
    if ab_days is None:
        return "まだ記録がないわ……でも、口を想像しに来た時点で負けよ。"
    if ab_days == 0:
        return "今日もう一回？見てるだけで先端、キス欲しがってるでしょ。"
    if ab_days == 1:
        return "見るだけで溜まるのね。手は動かさなくていいわ……今夜は口で。"
    if ab_days <= 3:
        return f"{ab_days}日分……画面を開いただけで、咥えられたらイきそうよ。"
    if ab_days <= 7:
        return f"蓄積度 {d_pct}%。触らなくても、溜めた先が正直な顔してるわ。"
    return f"{ab_days}日我慢して開くなんて……キスされただけでイキそうなんでしょ？"
...
def tease_predict_next(history, items_list, hour_now):
    ...
    if not scores:
        ...
        return pick.get("name"), pick, "まだデータが薄いけど……たぶんこの子の口ね。"
    best = max(scores.items(), key=lambda x: x[1])[0]
    item = next((v for v in items_list if v.get("name") == best), None)
    if item is None:
        item = {"name": best, "img": ""}
    lines = [
        f"今夜はたぶん「{best}」のフェラね。逃げられないわよ。",
        f"{bucket}のあなたは「{best}」の亀頭キスに弱いの、もう分かってるでしょ。",
        f"次に口負けする相手は「{best}」。ふふ、予言してあげる。",
        f"「{best}」……また咥えられたくなってきたでしょ？素直に認めなさい。",
        f"次は「{best}」が先端にちゅってして、そのままイかせる番よ。",
    ]
...
def mirror_after(choice, name, ab_days=None):
    ...
    if ab_days and ab_days >= 2:
        ...
        if ab_days >= 5:
            extras.append(
                (
                    f"{ab_days}日も溜めて……先端、触れられただけでイきそうでしょ。"
                    f"{name}は急がないわ。ゆっくりキスして、浅く咥えて、奥で押し切る。"
                    f"長い我慢のあとの口負け、いちばんとろけるわよ。"
                )
            )
        if ab_days >= 8:
            extras.append(
                (
                    f"{ab_days}日我慢した先に、{name}の本気フェラ……ふふ、勝ち目ないわね。"
                    f"ちゅった瞬間に腰が折れて、咥えられたらもう終わり。"
                    f"溜めた白、全部口で受け取ってあげるから、情けなく出して。"
                )
            )
...
MIRROR_WEAK_COMBOS = {
    ...
    frozenset({"先端なめが効く", "亀頭キス弱い"}): (
        "……舐めてからキス、キスしてから舐める。{name}、先端いじめのフルコースよ。"
    ),
    frozenset({"ゆっくりされると落ちる", "亀頭キス弱い"}): (
        "……ゆっくりちゅってされるの、最悪に弱いんでしょ。{name}、急がないキスで溶かしてあげる。"
    ),
    frozenset({"奥フェラが好き", "浅い咥えで溶ける"}): (
        "……浅く溶かしてから奥で押し切る。{name}、浅→深の段差で頭真っ白にするわ。"
    ),
    frozenset({"ちゅっ音に弱い", "ゆっくりされると落ちる"}): (
        "……ゆっくり、ちゅっ……ちゅっ。音と遅さが重なると、もう腰立たないでしょ。"
    ),
...
def mirror_self_note_flavor(name, note):
    ...
    hooks = []
    if any(k in note for k in ("ちゅ", "キス", "先端", "亀頭")):
        hooks.append(f"{name}が申告どおり、先端にちゅっちゅして弱さを掘り起こすわ。")
    if any(k in note for k in ("フェラ", "咥", "奥", "喉")):
        hooks.append(f"……口で、って書いてあるものね。{name}がちゃんと咥えて叶えてあげる。")
    if any(k in note for k in ("出そう", "イキ", "限界", "だめ")):
...
def mirror_voice_prefix(voice, name):
    ...
    n = nj(name)
    return {
        "sweet": f"（甘い声で）……ねえ、{n}の口で優しくしてあげる。",
        "sticky": f"（ねっとりと）……ふふ、{n}の口は離さないわよ。",
        "urgent": f"（急かすように）ほら、早くとろけなさい。待たないわよ——{n}の番なんだから。",
        "tease": f"（からかうように）ふふ……また{n}の前で、情けない顔しに来たの？",
        "dote": f"（溺愛混じりに）……いい子ね。{n}の口で、たっぷり可愛がってあげる。",
...
def mirror_weak_flavor(name, tags, loss_count=0, self_note=""):
    ...
    soft = {
        ...
        "ちゅっ音に弱い": f"……ちゅっ、って音だけでもう腰が引けるんでしょ。{name}にはバレてるわ。",
...
def mirror_permit_lines(name, stage="deny"):
    ...
    if stage == "grant":
        return random.choice([
            ...
            (
                f"はい、出していいわ。……{name}が奥まで咥えて、キスでふさいだまま受け止める。"
                f"ビクビクしながら口に出すの、いちばんかわいいわよ。"
            ),
            (
                f"許可ね。腰の力、抜いて。"
                f"{name}の唇が先を甘く包んで、ちゅっ……そのままイかせるわ。"
                f"敗北射精、ちゃんと味わいなさい。"
            ),
        ])
    if stage == "mouth":
        return random.choice([
            ...
            (
                f"口プレイ解禁よ。……深くても浅くてもいいわ。キスもしていい。"
                f"でも射精は禁止。{name}の中で限界直前まで溶かされなさい。"
            ),
        ])
    return random.choice([
        (
        ...
        (
            f"だめ。許可、出さないわ。"
            f"{name}はちゅってして離して、また咥えて離す。……イキたい気持ち、貯めなさい。"
        ),
        (
            f"まだよ。……先がビクビクしてても出させない。"
            f"{name}のキスと浅いフェラで、ふちのままとろけていなさい。"
        ),
    ])
...
def mirror_afterglow_lines(name, tags=None, loss_count=0):
    ...
    lines = [
        ...
        (
            f"びくびくしてる先に、またちゅっ。……ふふ、敏感すぎ。"
            f"{name}は余韻を逃さないわ。軽く咥えて、ゆっくり舐めて、またキス。"
            f"射精した直後の弱さ、大好きよ。"
        ),
        (
...
def mirror_step_lines(step_key, name, tags=None, loss_count=0, self_note=""):
    ...
    pool = {
        "kiss_only": [
            ...
            (
                f"音だけでも効くわよ。……ちゅっ。{name}のキスが先に吸い付いて、離さない。"
                f"咥えてほしそうな顔、かわいい。でもまだキスだけ。……とろけなさい。"
            ),
        ],
        "lick": [
            (
            ...
            (
                f"ねっとり、ねっとり。……{name}の舌が溝をなぞって、時々キス。"
                f"舐めてるだけなのに腰が砕けるの、認める？次で口に入れるわよ。"
            ),
        ],
        "shallow": [
            (
            ...
            (
                f"浅いのに熱い。……{name}が先だけをちゅうちゅうして離さない。"
                f"奥を欲しがっても、まだあげないわ。浅さでイきそうになるまで溶かしてあげる。"
            ),
        ],
        "deep": [
            (
            ...
            (
                f"奥、きついでしょう。……でも抜かないわ。"
                f"{name}が深く咥えたまま、息の合間に先端を舌でつ付く。"
                f"ふふ、喉に負けそうな顔……好きよ。仕上げ、もうすぐね。"
            ),
        ],
        "finish": [
            (
            ...
            (
                f"もう逃がさないわ。……{name}が咥えたまま、キスで塞いで、リズムを速める。"
                f"限界のビクビクを全部口で受け止めるから、許可出したらイっていいのよ。"
            ),
        ],
    }
...
def mirror_gauge_lines(gauge_key, name, tags=None, loss_count=0, self_note=""):
    ...
    pool = {
        "touch": [
            ...
            (
                f"まだ手の段階。……でも先端、もうキス欲しがってるわよ。"
                f"{name}の唇を想像しながら、ゆっくり温めて。次は口に渡すわ。"
            ),
        ],
        "near": [
            (
...
if all_names:
    ...
    if st.session_state.get("gacha_result"):
        ...
        g_tags = list(g_item.get("weak_tags") or [])
        tag_bit = f"（弱点：{'・'.join(g_tags[:2])}）" if g_tags else ""
        gacha_msgs = [
            f"今日の弱点は……「{g_name}」！もう口から逃げられないわ。{tag_bit}",
            f"運命が決まったわ。今日は「{g_name}」のフェラにとろかされなさい。",
            f"「{g_name}」があなたを選んだわ。先端、ちゅってされに行きなさい。",
            f"ふふ、「{g_name}」ね。咥えられても抵抗、無駄よ。",
            f"今日の決め技は「{g_name}」の亀頭キス。……素直にイきなさい。",
            f"「{g_name}」の口で負けなさい。{tag_bit}記録、あとでつけるのよ。",
        ]
