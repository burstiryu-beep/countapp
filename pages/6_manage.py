...
with st.form("add"):
    ...
    tab = st.selectbox("カテゴリ", tab_ids, format_func=lambda x: tab_names[x])
...
if data["items"]:
    ...
    if st.button("名前を変更", key="rename_btn"):
        if not new_name.strip():
            ...
            st.warning("同じ名前です")
        elif rename_item(data, data["items"][rename_key]["name"], data["items"][rename_key]["tab"], new_name.strip()):
            save_data(data)
            st.success("変更しました")
            st.rerun()
        else:
...
# ===== タブ追加 =====
st.markdown("<h3>🏷 カテゴリを追加</h3>", unsafe_allow_html=True)

new_tab = st.text_input("カテゴリ名", key="new_tab_input")
if st.button("追加する", key="add_tab_btn"):
    if new_tab.strip():
        tab_id = new_tab.strip()
        if any(t["id"] == tab_id for t in data["tabs"]):
            st.error("同じIDのカテゴリが既にあります")
        else:
...
# ===== タブ編集 =====
st.markdown("<h3>✏️ カテゴリを編集</h3>", unsafe_allow_html=True)

if editable_tabs:
    edit_tab = st.selectbox(
        "編集するカテゴリ", editable_tabs,
        format_func=lambda t: t["name"], key="edit_tab_sel"
    )
    new_display_name = st.text_input("表示名", value=edit_tab["name"], key="edit_tab_name")
    new_tab_id = st.text_input("タブID", value=edit_tab["id"], key="edit_tab_id")

    col1, col2 = st.columns(2)
    with col1:
        if st.button("更新する", key="update_tab_btn"):
            updated = False
            ...
            if updated:
                ...
                st.info("変更がありません")
    with col2:
        if st.button("削除する", key="delete_tab_btn"):
            if delete_tab(data, edit_tab["id"]):
                save_data(data)
                st.success("カテゴリと関連データを削除しました")
                st.rerun()
else:
    st.caption("編集可能なカテゴリがありません")
